...
# PU占比
sql = "select sum(case when pay_price > 0 then 1 else 0 end) as `付费用户`,sum(case when pay_price > 0 then 0 else 1 end) as `非付费用户` from age_of_barbarians"
data = pd.read_sql(con=engine.connect(), sql=text(sql))
c1 = (Pie().add(
    "",
...
# DNU柱形图
sql = """select cast(register_time as date) day,count(1) dnu from
age_of_barbarians
group by cast(register_time as date)
order by day;"""
data = pd.read_sql(con=engine.connect(), sql=text(sql))
c2 = (Bar().add_xaxis(list(data['day'])).add_yaxis('新增用户数', list(
    data['dnu'])).set_global_opts(title_opts=opts.TitleOpts(
        title='每日薪资用户数量')).render('bar_dnu.html'))
os.system('bar_dnu.html')

# 每小时注册情况
sql = """select hour(cast(register_time as datetime)) as hour,
count(1) as dnu
from age_of_barbarians
group by hour(cast(register_time as datetime))
order by hour;"""
data = pd.read_sql(con=engine.connect(), sql=text(sql))
c3 = (Line().add_xaxis(list(data['hour'])).add_yaxis('新增用户数', list(
    data['dnu'])).set_global_opts(title_opts=opts.TitleOpts(
        title='每小时新增用户数量')).render('line_dnu.html'))
...
"""
data = pd.read_sql(con=engine.connect(), sql=text(sql))
c4 = (Bar().add_xaxis(list(data.columns)).add_yaxis(
    "平均在线时长(单位：分钟)", list(data.values[0])).set_global_opts(
        title_opts=opts.TitleOpts(title="平均在线时长")).render("bar_online.html"))
...
"""
data = pd.read_sql(con=engine.connect(), sql=text(sql))
c5 = (Liquid().add("lq", [data['rate'][0], data['rate'][0]]).set_global_opts(
    title_opts=opts.TitleOpts(title="付费比率")).render("liquid_base.html"))
...
data = pd.read_sql(con=engine.connect(), sql=text(sql))
c6 = (Bar().add_dataset(
    source=[data.columns.tolist()] + data.values.tolist()).add_yaxis(
        series_name="平均胜率",
        y_axis=[]).add_yaxis(series_name="付费用户胜率", y_axis=[]).add_yaxis(
            series_name="非付费用户胜率", y_axis=[]).set_global_opts(
...
c7 = (Bar().add_dataset(
    source=[data.columns.tolist()] + data.values.tolist()).add_yaxis(
        series_name="平均场次",
        y_axis=[]).add_yaxis(series_name="付费用户平均场次", y_axis=[]).add_yaxis(
            series_name="非付费用户平均场次", y_axis=[]).set_global_opts(
